...
def get_license_objects(lic):
    ...
    if isinstance(lic, list):
        return [get_license_objects(item) for item in lic]
    else:
        iri = license_iri_map.get(lic)
        return {
            "label": lic,
...
def parse(json_obj):
    ...
    parsed = {
        ...
        "dct_spatial": geonames_iri_from_code(json_obj.get("region")),
        "dct_language": linguistic_system_iri(json_obj.get("language")),
        "dct_created": json_obj.get("created_at"),
        "dct_modified": json_obj.get("last_modified"),
        "dct_description": json_obj.get("description"),
        "dct_license": get_license_objects(json_obj.get("license")),
        "dct_format": get_media_type(json_obj.get("format")),
        "dct_accessRights": "private" if json_obj.get("private") else "public",
        "dct_source": get_source_datasets(json_obj.get("source_datasets")),
        "dct_size": json_obj.get("size_categories"),
        ...
        "hasModality": get_modality_concept(json_obj.get("modality")),
        "dct_subject": json_obj.get("tags", None),

        # Scientific Articles
        "hasAcademicArticle": {
            "doi": json_obj.get("doi"),
            "arxiv":  json_obj.get("arxiv"),
            "paperswithcode_id": json_obj.get("paperswithcode_id")
        },
    
        # Annotations
        "hasAnnotation": {
            "linguisticMethod": json_obj.get("language_creators"),
            "annotationMethod": json_obj.get("annotations_creators"),
            "dct_title": json_obj.get("annotation")
        },

        # Usage
        "hasUsage": {
            "hasTask": json_obj.get("task_ids"),
            "hasTaskCategory": json_obj.get("task_categories")     
        }
    }
