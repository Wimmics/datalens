...
def parse(json_obj: dict[str, Any]) -> dict[str, Any]:
    ...
    parsed["views"] = normalize_string(parsed.get("Model_TotalViews"))
    parsed["kernels"] = normalize_string(parsed.get("Model_TotalKernels"))

    tags_raw = parsed.get("Model_Tags", "") or ""
    ...
    license_tokens = normalize_string(parsed.get("LicenseName")) or ""
